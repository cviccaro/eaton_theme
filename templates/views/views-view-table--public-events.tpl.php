      ...
      <tr <?php if ($row_classes[$row_count]) {
        ...
        <?php foreach ($row as $field => $content) : ?>
          <td <?php if ($field_classes[$field][$row_count]) {
            ...
            switch ($field) {
                case 'nothing':
                    $visibility = isset($row['field_event_visibility']) ? $row['field_event_visibility'] : 'Public';
                    $is_owner = _eaton_backbone_user_owns_event($content, $user);
                    if ($visibility == 'Public') {
                        print l('Public Registration', 'register/' . $content);
                        if ($is_coordinator || $is_owner) {
                             print '<br />' . l('Preregistration', 'event/' . $content);
                        }
                    } elseif ($visibility == 'Private') {
                        if ($is_coordinator || $is_owner) {
                            print l('Preregistration', 'event/' . $content);
                        }
                    }
                    break;
                default:
